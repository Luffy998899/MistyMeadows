...
export default async function HomePage() {
  const [settings, rooms, facilities, dining, testimonials] = await Promise.all([
    getSettings(),
    getRooms(),
    getFacilities(),
    getDining(),
  ...
  return (
    <>
      ...
      {/* Facilities */}
      {amenities.length > 0 ? (
        <section className="section bg-paper-warm" aria-labelledby="facilities-title">
          <div className="shell">
            <SectionHeading
        ...
        <section className="section" aria-labelledby="dining-title">
          <div className="shell grid items-center gap-10 lg:grid-cols-2 lg:gap-20">
            ...
            <div>
              <SectionHeading
                eyebrow="Dining"
                title={<span id="dining-title">Thalis, served all day</span>}
                ...
                {thalis.map((item) => (
                  <li key={item.id} className="border-b border-paper-edge py-5">
                    <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
                      ...
                      {item.price_note ? (
                        <p className="text-sm text-burgundy">{item.price_note}</p>
                      ) : null}
                    </div>
      ...
      <section className="on-pine section" aria-labelledby="enquire-title">
        <div className="shell max-w-3xl">
          <p className="eyebrow">Reservations</p>
          <h2 id="enquire-title" className="text-h1 mt-5">
            Tell us when you would like to come <span className="signature">up</span>
          </h2>
